  ...
  if (queue.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
        ...
        <p className="font-body text-sm">No speakers in queue</p>
      </div>
    );
  }
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground flex items-center gap-1">
                  ...
                  {isSpeaking ? 'Speaking now' : `Position #${index + 1}`}
                </p>
              </div>
              ...
              {isAdmin && (
                <div className="flex gap-1">
                  {isSpeaking && onSkip && (
                    <Button variant="ghost" size="icon" onClick={() => onSkip(entry.id)} className="h-8 w-8">
                      <SkipForward className="w-4 h-4" />
                    </Button>
                  )}
                  {onRemove && (
                    <Button variant="ghost" size="icon" onClick={() => onRemove(entry.id)} className="h-8 w-8 text-destructive">
                      <X className="w-4 h-4" />
                    </Button>
                  )}
                </div>
