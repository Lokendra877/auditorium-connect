...
export function AnalyticsPanel({ analytics }: AnalyticsPanelProps) {
  const stats = [
    ...
    {
      label: 'Completed',
      value: analytics.completedSpeakers,
      icon: Mic2,
      color: 'text-success',
      bg: 'bg-success/10',
    },
    {
      label: 'Skipped',
      value: analytics.skippedSpeakers,
      icon: SkipForward,
      color: 'text-warning',
      bg: 'bg-warning/10',
    },
    {
      label: 'Waiting',
      value: analytics.waitingSpeakers,
      icon: Clock,
      color: 'text-accent',
      bg: 'bg-accent/10',
    },
  ...
  const timeStats = [
    {
      ...
      value: formatDuration(analytics.averageSpeakingTime),
      icon: TrendingUp,
    },
    {
      label: 'Total Speaking Time',
      value: formatDuration(analytics.totalSpeakingTime),
      icon: Timer,
    },
    {
  ...
  // Recent speakers log (last 5 completed/done)
            ...
            <div className="space-y-1">
              {recentSpeakers.map((entry) => {
                const duration = entry.started_speaking_at && entry.finished_speaking_at
                  ...
                  : 0;
                return (
                  <div key={entry.id} className="flex items-center justify-between text-xs py-1.5 px-2 rounded bg-muted/50">
